    ...
    function getEntries($criteria = false)
    {
        if (!isset($this->_entries)) {
            $this->_entries = $this->entries->annotate(array(
                'has_attachments' => SqlAggregate::COUNT(
                    SqlCase::N()
...
implements TemplateVariable
{
    ...
    function getBody()
    {
        if (!isset($this->_body)) {
            ...
            $this->_body = ThreadEntryBody::fromFormattedText(
                $body,
                $this->format,
    ...
    function getEmailHeaderArray()
    {
        ...
        if (
            !isset($this->_headers) && $this->email_info
    ...
    protected function normalizeFileInfo($files, $add_error = true)
    {
        ...
        foreach ($files as $id => $info) {
            ...
            elseif ($add_error) {
                $error = $info['error']
                    ?: sprintf(
                        _S('Unable to save attachment - %s'),
                        $info['name'] ?: $info['id']
                    );
    ...
    */
    ...
    function getAttachmentsLinks($file = 'attachment.php', $target = '_blank', $separator = ' ')
    {
        ...
        foreach ($this->attachments as $att) {
            ...
            $str .= sprintf(
                ...
                $att->file->getDownloadUrl(),
                $target,
                Format::htmlchars($att->file->name),
                $size,
                $separator
            );
        }
    ...
    static function create($vars = false)
    {
        ...
        $entry = new static(array(
            ...
            'source' => $vars['source'],
            'flags' => $vars['flags'] ?: 0,
            'sig_img' =>  $vars['sig_img'] ?: null,
            'sig_name' =>  $vars['sig_name'] ?: null,
            'sig_date' => $vars['sig_date'] ?: null,
            'sig_assignId' => $vars['sig_assignId'] ?: null
        ));
            ...
            function ($m) use ($self, $thisstaff, $cfg, $hideName, $mode) {
                switch ($m['key']) {
                    ...
                    case 'timestamp':
                        $timeFormat = null;
                        if (
                            $mode != self::MODE_CLIENT && $thisstaff
                            && (!isset($_REQUEST['a']) || $_REQUEST['a'] != 'print')
                            && !strcasecmp(
                                $thisstaff->datetime_format,
                                'relative'
                            )
                        ) {
        ...
        switch (true) {
            ...
            case isset($data['fields']):
                ...
                foreach ($data['fields'] as $id => $f) {
                    ...
                    $changes[] = sprintf(
                        '<strong>%s</strong> %s',
                        $field->getLocal('label'),
                        $impl->whatChanged($before, $after)
                    );
                }
                    ...
                    if (isset($data[$f])) {
                        ...
                        if ($old and $new) {
                            $changes[] = sprintf(
                                ...
                                Format::htmlchars($old),
                                Format::htmlchars($new)
                            );
                        } elseif ($new) {
                            $changes[] = sprintf(
...
class MergedEvent extends ThreadEvent
{
    ...
    {
        return sprintf(
            $this->template(__('<b>{somebody}</b> merged this ticket with %s{data.id}%s<b>{data.ticket}</b>%s {timestamp}'), $mode),
...
class LinkedEvent extends ThreadEvent
{
    ...
    {
        return sprintf(
            $this->template(__('<b>{somebody}</b> linked this ticket with %s{data.id}%s<b>{data.ticket}</b>%s {timestamp}'), $mode),
            '<a href="tickets.php?id=',
            '">',
            '</a>'
        );
...
class UnlinkEvent extends ThreadEvent
{
    ...
    {
        return sprintf(
            $this->template(__('<b>{somebody}</b> unlinked this ticket from %s{data.id}%s<b>{data.ticket}</b>%s {timestamp}'), $mode),
            '<a href="tickets.php?id=',
            '">',
            '</a>'
        );
...
class ThreadEntryBody /* extends SplString */
{
    ...
    function convertTo($type)
    {
        ...
        switch ($conv) {
            case 'text:html':
                return new ThreadEntryBody(sprintf(
                    '<pre>%s</pre>',
                    Format::htmlchars($this->body)
                ), $type);
            case 'html:text':
    ...
    function stripQuotedReply($tag)
    {
        ...
        if ((list($msg) = explode($tag, $this->body, 2)) && trim($msg)) {
            ...
            if ($images_before) {
                ...
                $this->stripped_images = array_diff(
                    $images_before[2],
                    $images_after[2]
                );
            }
...
class ResponseThreadEntry extends ThreadEntry
{
    ...
    static function add($vars, &$errors = array())
    {
        ...
        elseif (!$vars['signature_name'] && !$vars['signature_date'] && !$vars['response'])
        ...
        $numericPart = filter_var($vars['assignId'], FILTER_SANITIZE_NUMBER_INT);
        $numericValue = (int)$numericPart;
        $info=array();
        $query = 'select * from ' . TABLE_PREFIX . 'staff where staff_id = ' . $numericValue;
        $tes = db_query($query);
        if (db_num_rows($tes)) {
            $info = db_fetch_array($tes);
        }
        if ($vars['signature_name'] && $vars['signature_date']) {
            $vars['sig_name'] = $vars['signature_name'];
            $vars['sig_date'] = $vars['signature_date'];
            $vars['sig_assignId'] = $info['firstname'] . ' ' . $info['lastname'];
        }
