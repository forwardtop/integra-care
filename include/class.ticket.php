...
class Ticket extends VerySimpleModel implements RestrictedAccess, Threadable, Searchable
{
    ...
    public function getUpdateInfo()
    {
        ...
        $info = array(
            ...
            'email' => $this->getEmail(),
            'source_other' => $this->getExtraData('source_other'),
            'topicId' => $this->getTopicId(),
            'deptId' => $this->getDeptId(),
            'topicId' => $this->getTopicId(),
            'slaId' => $this->getSLAId(),
            'phone' => $this->getPhoneNumber(),
            'phone_ext' => $this->getExtraData('phone_ext'),
            // --- start coderXO mod --- //
            ...
            'topicId' => $this->getExtraData('topic_id'),
    ...
    public function getRelatedTickets()
    {
        return sprintf(
            ...
            $this->getDeptName(),
            $this->getAssignee(),
            Format::datetime($this->getCreateDate())
        );
    }
    ...
    public function transfer($deptId, $comments, $alert = true)
    {
        ...
        $title = sprintf(
            _S('Ticket transferred from %1$s to %2$s'),
            $currentDept,
            $this->getDeptName()
        );
    ...
    public function assign($assignId, $note, $alert = true)
    {
        global $thisstaff;
    ...
    public function postReply($vars, &$errors, $alert = true, $claim = true)
    {
        ...
        if (true) {
            ...
            if (isset($vars['signature_name']) && !empty($vars['signature_name'])) {
               // $msg = $tpl->getReplySignatureTemplate();
                $variables['ticket.account_num'] = 11111;
                $tranding_name = $this->getExtraData('company');
                $tId  = $this->getNumber();
                $email1  = $this->getEmail();
                $email2 =  $this->getExtraData('email2');
                $name  = $this->getName();
                $firstName = strpos($name," ") > 0 ? substr($name,0, strpos($name," ")): $name;
                $lastName = strpos($name," ") > 0 ? substr($name, strpos($name," ")+1): "";
                $mobile = $this->getExtraData('mobile');
                $phone = $this->getPhoneNumber() ;
                $add1  = $this->getExtraData('address');
                $add2  = $this->getExtraData('address1');
                $suburb = $this->getExtraData('suburb');
                $state = $this->getExtraData('state');
                $postcode =$this->getExtraData('postcode') ;
                $ticketDate = $this->getCreateDate();
                $tickeetStatus = $this->isCloseable() ? 'closed' : 'opend' ;
                $source = $this->getSource();
                $priority = $this->getPriority() ;
                $assigned = $this->isAssigned()  ? $this->getAssignees() : 'Unassigned' ;
                $sla  = $this->getSLA();
                $vip =  $this->getExtraData('vip')  ;
                $department = $this->getDeptName() ;
                $helpTopic = $this->getHelpTopic() ;
                $sub  = $this->getSubject();
                $variables['ticket.tranding_name'] = "tranding";
            ...
            if (isset($vars['signature_name'])) {
               $query = 'SELECT * FROM ' . TABLE_PREFIX . 'thread_entry ORDER BY created DESC LIMIT 1';
                $tes = db_query($query);
                if (db_num_rows($tes)) {
                ...
                $ename=$info['sig_name'];
                $edate=$info['sig_date'];
                $eassign=$info['sig_assignId'];
                $eimg=$info['sig_img'];
                $url = 'https://integra-care.com.au/scp/' . $eimg;

                $msg['body'] =
                 "
                 <div style='border:1px solid #ddd;border-top:none;border-bottom:2px solid #aaa;border-radius:0 0 5px 5px;'>
                 </div>
                 <div>
                 <p style='color:black'><strong>Delivery & Acceptance</strong></p>
                 </div>
                 <p style='width:50%;'>I,  $ename hereby confirm that Integra engineer/representative has attended the site location as per the address details outlined above in this ticket. The Integra engineer/representative has successfully completed the work and/or resolved the fault, and/or delivered the goods and/or services specified in this ticket. We acknowledge that the work and/or delivery has been executed to our satisfaction. Furthermore, we agree to fulfill the financial obligations by paying for the products delivered and the associated fees and charges mentioned in this ticket.</p>
                 <img src='$url' style='height:40px;width:100%;border:1px solid #dfdddd:border-radius:12px;'>
                 <br>
                 <p>
                 Date: <small>$edate</small>
                 Integra Engineer or Representative Name: $eassign
                 </p>
                ";

                if (!empty($vars['signature_name'])) {
                    $email->send(
                        $recipients,
                        $msg['subj'],
                        $msg['body'],
    ...
    public function logEvent($state, $data = null, $user = null, $annul = null, $staff = null)
    {
        switch ($state) {
            case 'collab':
            case 'transferred':
    ...
    public function update($vars, &$errors)
    {
        ...
        foreach ($forms as $form) {
            ...
            if (!in_array($form->getObjectId(), $vars)) {
                continue;
            }
            // echo var_dump($form);
        ...
        $sql = 'UPDATE ' . TICKET_TABLE . ' SET updated=NOW() '
            ...
            // --- start coderXO mod --- //
            . ' ,source_other=' . db_input($vars['source_other'])
            . ', suburb = ' . db_input($vars['suburb']) .
            ', dept_id = ' . db_input($vars['deptId']) .
            ', topic_id = ' . db_input($vars['topicId']) .
            ', staff_id = ' . db_input((int)$assignId) .
            ', carrier = ' . db_input($vars['carrier']) .
            ', carrier_ref = ' . db_input($vars['carrier_ref']) .
            ', address = ' . db_input($vars['address']) .
            ', address2 = ' . db_input($vars['address2']) .
            ', state = ' . db_input($vars['state']) .
            ', postcode = ' . db_input($vars['postcode']) .
            ', company = ' . db_input($vars['company']) .
            ', mobile = ' . db_input($vars['mobile']) .
            ', contract = ' . db_input($vars['contract']) .
            ', vip = ' . db_input($vars['vip']) .
            ', email2 = ' . db_input($vars['email2'])
            // --- end coderXO mod --- //
        ...
        $priorityId = $vars['-field-id-22'][0];
        $summary = $vars['-field-id-20'];
        // echo $priorityId;
        // print_r($vars);


        if ($priorityId > 0) {
            $sql3 = "SELECT A1.`id` FROM `" . TABLE_PREFIX . "form_entry` A1 LEFT JOIN `" . TABLE_PREFIX . "form` A2 ON (A1.`form_id` = A2.`id`) WHERE A1.`object_id` = " . db_input($this->getId()) . " AND A1.`object_type` = 'T'";
            ...
            if ($result) {
                ...
                $sql4 = "SELECT priority_desc FROM `" . TABLE_PREFIX . "ticket_priority`  WHERE `priority_id` = $priorityId";
                $result = db_query($sql4);

                if ($result) {
                    // Fetch the result as an associative array
                    $row = db_fetch_array($result);
                    $priorityValue = $row['priority_desc'];

                    $sql2 = "UPDATE " . TABLE_PREFIX . "form_entry_values
                SET `value` = '$priorityValue', `value_id` = $priorityId
                WHERE (`entry_id` = $formid AND `field_id` = 22)
                LIMIT 1";
                    // echo $sql2;
                    // die();
                    ...
                    $sql22 = "UPDATE " . TABLE_PREFIX . "form_entry_values
                SET `value` = '$summary', `value_id` = 0
                WHERE (`entry_id` = $formid AND `field_id` = 20)
                LIMIT 1";
            ...
            $sql3 = 'UPDATE ' . TABLE_PREFIX . 'ticket__cdata SET priority=' . db_input($priorityId);

            $sql3 .= ' WHERE ticket_id=' . db_input($this->getId());
            if (!db_query($sql3))
                return false;

            if ($this->getUserId()) {
                $sql4 = 'UPDATE ' . TABLE_PREFIX . 'user SET name=' . db_input($vars['name']);

                $sql4 .= ' WHERE id=' . db_input($this->getUserId());
                // echo  $sql4;
                // die();
                if (!db_query($sql4))
                    return false;
        ...
        //if(!db_query($sql2) || !db_affected_rows())
    ...
    public static function create(
        ...
        $ticket = new static(array(
            ...
            'vip' => $vars['vip'],
            'source_other' => $vars['source_other'],
            'mobile' => $vars['mobile'],
            'carrier' => $vars['carrier'],
            'carrier_ref' => $vars['carrier_ref'],
            'mobile' => $vars['mobile'],
        ));
