...
<div class="thread-entry <?php
    ...
    <div class="header">
        ...
        echo sprintf(
            ...
            $name,
            sprintf(
                '<a name="entry-%d" href="#entry-%1$s"><time %s
    ...
    <div class="thread-body no-pjax">
        <div>
            <?= $entry->getBody()->toHtml() == '(empty)' && !empty($entry->sig_name) ?
                "<p><strong>Delivery & Acceptance</strong></p>
    <p>I, <b> $entry->sig_name </b> Hereby confirm that Integra engineer/representative has attended the site location as per the address details outlined above in this ticket. The Integra engineer/representative has successfully completed the work and/or resolved the fault, and/or delivered the goods and/or services specified in this ticket.
    We acknowledge that the work and/or delivery has been executed to our satisfaction. Furthermore, we agree to fulfill the financial obligations by paying for the products delivered and the associated fees and charges mentioned in this ticket.</p>
    <img src='$entry->sig_img' height='40px' width='100%' style='border:1px solid #dfdddd; border-radius:12px; ' /><br/>
    <p>Date:<small>" . date('d/m/Y H:i:s', strtotime($entry->sig_date)) . "</small> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Integra Engineer or Representative Name: $entry->sig_assignId</p>"
                : $entry->getBody()->toHtml(); ?>
        </div>
