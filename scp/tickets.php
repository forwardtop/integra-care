        ...
        switch (strtolower($_POST['a'])):
           ...
           case 'signature':
                ...
                if (!$role || !$role->hasPerm(Ticket::PERM_REPLY)) {
                    ...
                    $vars = $_POST;
                    $vars['files'] = $response_form->getField('attachments')->getFiles();
                    $vars['sig_img'] = $filename;
                    $vars['signature_name'] = ThreadEntryBody::clean($vars['signature_name']);
                    if (!$vars['signature_name']) {
                        $errors['signature_name'] = __('Response required');
                    }
                ...
                if (!$errors && ($response = $ticket->postReply(
                    ...
                    $msg = sprintf(
                        __('%s: Signature posted successfully'),
                        sprintf(
                            __('Ticket #%s'),
                            sprintf(
                                '<a href="tickets.php?id=%d"><b>%s</b></a>',
